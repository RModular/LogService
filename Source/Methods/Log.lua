...
local SendToDiscord = require(script.Parent.Parent.Functions:WaitForChild('SendLogToDiscord'))
...
function Module:Log(message: string, logLevel: Types.LogType?, outputDetails: boolean?, sendToDiscord: boolean?): nil
	-- Default values
	local Message: string = message
	local Level: Types.LogType = logLevel or "Info"
	local Details: boolean = outputDetails or false
	local SendToDiscord = sendToDiscord or false
	...
	if SendToDiscord then
		Module.SendLogToDiscord(LogEntry)
	end
